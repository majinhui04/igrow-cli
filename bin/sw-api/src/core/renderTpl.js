...
function renderTpl(swconfig, swaggerFile) {
    ...
    let tagsArr = swaggerFile.tags.map((item, index) => {
        let CasePath = mapFileName[index]
            ? mapFileName[index]
            : item.description.replace(/\s(\w)/g, ($1, $2) => $2.toUpperCase());
        return {
            ...item,
            filePath: `${outputPath}/${
                isRenderTypescript ? CasePath + "/index" : CasePath
            }.${isRenderTypescript ? "ts" : "js"}`,
    ...
    if (Reflect.ownKeys(filterName).length) {
        tagsArr = Reflect.ownKeys(filterName).reduce((pre, key, idx) => {
            if (tagsArr[key]) {
                ...
                //  .match(/.*\/(.*?)\/index.ts/)
                tempItem.tsTypePath = tempItem.tsTypePath.replace(
                    tempItem.casePath,
                    filterName[key]
                );
                tempItem.filePath = tempItem.filePath.replace(
                    tempItem.casePath,
                    filterName[key]
                );
    ...
    Object.keys(swaggerFile.paths).forEach((path) => {
        Object.keys(swaggerFile.paths[path]).forEach((method) => {
            ...
            if (pathKey[method]) {
                Object.keys(swaggerFile.paths[path][method]).forEach(
                    (pathsKey) => {
                        if (pathsKey === "tags") {
                            tagsArr.forEach((item) => {
                                const obj = {};
                                if (
                                    swaggerFile.paths[path][method][
                                        pathsKey
                                    ].includes(item.name)
                                ) {
            ...
            item.children.forEach((pathObj) => {
                Object.keys(pathObj).forEach((typeKey) => {
                    if (pathKeyArr.includes(typeKey)) {
                        item.tsTypeTempArr.push(
                            renderApiName(pathObj, typeKey) + "Props"
                        );
                        item.tsReturnTypeArr.push(
                            renderApiName(pathObj, typeKey) + "ReturnTypes"
                        );
                    }
                });
            });
                ...
                if (pathKeyArr.includes(typeKey)) {
                    if (pathObj[typeKey]) {
                        ...
                        if (isRenderTypescript && item.isAxiosTypes) {
                            tempTpl.replace(
                                /\(\{\{params\}\}\)/g,
                                `({{params}}):Promise<AxiosResponse<${item.tsReturnTypeArr[idx]}>>`
                            );
                        ...
                        let { query, body, params } = renderParamsBodyAndQuery(
                            pathObj,
                            typeKey,
                            tempTpl,
                            isRenderTypescript ? item.tsTypeTempArr[idx] : "",
                            pathObj[typeKey].parameters,
                            definitions
                        );
                        ...
                        if (
                            isRenderTypescript &&
                            item.tsTypeTempArr &&
                            item.tsTypeTempArr.length > 0
                        ) {
                            // 渲染types.ts文件
                            renderTypeStr += renderTypesProps(
                                item.tsTypeTempArr[idx],
                                pathObj[typeKey].parameters,
                                definitions
                            );
                            renderTypeStr += renderReturnTypes(
                                item.tsReturnTypeArr[idx],
                                pathObj[typeKey].responses,
                                definitions
                            );
                        }
                    }
