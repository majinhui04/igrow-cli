...
const REACT_TPL = "majinhui04/vue-starter-pro";
...
class CliUtil {
    ...
    initializing(pkg) {
        const messages = [];
        messages.push(
            `🔥  Welcome to use igrow-cli ${chalk.grey(`v${pkg.version}`)}`
        );
        // messages.push(chalk.grey("https://github.com/igrow/igrow-cli"));
        //messages.push(chalk.grey("https://www.npmjs.com/package/igrow-cli"));
    ...
    checkVersion(pkg) {
        console.log();
        console.log("🛠️  Checking your igrow-cli version...");
        ...
        if (update) {
            const messages = [];
            messages.push(
                `Update available ${chalk.grey(update.current)} → ${chalk.green(
                    update.latest
                )}`
            );
            messages.push(
            ...
            console.log("🛠️  Finish checking your igrow-cli. CAUTION ↑↑", "⚠️");
        } else {
            checkResult = true;
            console.log(
                "🛠️  Finish checking your igrow-cli. OK",
                chalk.green("✔")
            );
        }
        return checkResult;
                ...
                .prompt([
                    {
                        ...
                        name: "ok",
                        message:
                            "Target directory is not empty and will overwritten. Continue?",
                    },
                ])
                .then((answers) => {
                    if (answers.ok) {
    ...
    downloadAndGenerate(template, to, appName) {
        const spinner = ora(
            `Download the template from ${GIT_BASE}${template}`
        );
        spinner.start();
        ...
        this.download(template, to)
            .then(() => {
                spinner.stop();
                console.log(
                    "🌟  Finish create a new react project. OK",
                    chalk.green("✔")
                );
                console.log();
                console.log(
